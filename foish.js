...
class FoishTank {
    ...
    step() {
        ...
        this.bigFoish.loopFoish(this.w, this.h);
        ...
        for (let i = 0; i < this.foishes.length; i++) {
            ...
            this.turnFactor = clamp(this.turnFactor, -0.3, 0.3);
...
class Foish {
    ...
    toroidalDistance(other, w, h) {
        const [dx, dy] = this.diff(other, w, h);
...
function nanCheck(foish, n) {
    if (isNaN(foish.x)) {
        console.log(n + ' NaN x', foish);
    }
    if (isNaN(foish.y)) {
        console.log(n + ' NaN y', foish);
    }
    if (isNaN(foish.vX)) {
        console.log(n + ' NaN vX', foish);
    }
    if (isNaN(foish.vY)) {
        console.log(n + ' NaN vY', foish);
    }
